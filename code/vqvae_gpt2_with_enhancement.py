...
class VectorQuantizer(nn.Module):
    ...
    def _update_ema(self, flat_input, encoding_indices):
        ...
        self._ema_cluster_size.data.mul_(self._ema_decay).add_(
            (1 - self._ema_decay) * torch.sum(encodings, 0)
        )
        
        # Update EMA weights
        n = torch.sum(self._ema_cluster_size.data)
        self._ema_w.data.mul_(self._ema_decay).add_(
            (1 - self._ema_decay) * torch.matmul(encodings.T, flat_input)
        )
    ...
    def _compute_diversity_loss(self, encoding_indices):
        ...
        kl_div = torch.sum(usage_probs * torch.log(usage_probs / (target_probs + 1e-8) + 1e-8))
        
        # Alternative: variance-based diversity loss
        variance_loss = torch.var(usage_probs)
        
        return kl_div + variance_loss
    ...
    def _check_codebook_reset(self, encoding_indices):
        ...
        if self._reset_counter % self.reset_frequency == 0:
            total_usage = self._usage_counts.sum()
            if total_usage > 0:
                usage_ratio = self._usage_counts / total_usage
                unused_ratio = (usage_ratio < self.reset_threshold).float().mean()
    ...
    def _reset_codebook(self, flat_input):
        ...
        usage_ratio = self._usage_counts / (self._usage_counts.sum() + 1e-8)
        ...
        if unused_mask.sum() > 0:
            ...
            if flat_input.shape[0] >= num_unused:
                ...
                new_embeddings = flat_input[indices]
            else:
    ...
    def forward(self, inputs):
        ...
        regularization_loss = self._compute_regularization_loss(embedding_weight)
        ...
        # FIXED: Handle case where input_shape has only 1 dimension
    ...
    def get_codebook_stats(self):
        ...
        stats = {
            ...
            'usage_ratio': (self._usage_counts / (self._usage_counts.sum() + 1e-8)).clone().detach(),
...
class GPT2VQVAE(nn.Module):
    def __init__(self, vocab_size, d_model=768, num_embeddings=512, 
        ...
        # Vector Quantizer
        self.vector_quantizer = VectorQuantizer(num_embeddings, d_model, commitment_cost, ema_decay, diversity_gamma, reset_threshold, reset_frequency, use_ema)
