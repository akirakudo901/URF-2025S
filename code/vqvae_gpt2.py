...
class GPT2VQVAE(nn.Module):
    ...
    def decode(self, memory, prompt_sequences, cot_sequences, prompt_mask=None, cot_mask=None, 
        ...
        Args:
            ...
            prompt_sequences (torch.Tensor): Prompt sequences [batch_size, K]
            cot_sequences (torch.Tensor): Used as teacher-forcing template (inference=False) or for auto-regression (inference=True)
            prompt_mask (torch.Tensor, optional): Prompt attention mask for padding
            cot_mask (torch.Tensor, optional): COT attention mask for padding
            inference (bool): If True, performs auto-regressive generation. If False, performs teacher-forcing.
        ...
        #                                        cot_mask, inference, pad_token_id, use_caching)
    ...
    def _decode_embed_sum(self, chain_memory, prompt_sequences, cot_sequences, prompt_mask, cot_mask, inference, pad_token_id, use_caching):
        ...
        # Prompt embeddings
        if prompt_sequences is not None and K > 0:
            prompt_embeds = self.decoder.transformer.wte(prompt_sequences)  # [B, K, d_model]
            prompt_embeds = prompt_embeds.unsqueeze(1).expand(-1, M, -1, -1).reshape(B * M, K, d_model)  # [B*M, K, d_model]
        else:
            prompt_embeds = None

        if prompt_mask is not None and K > 0:
            prompt_mask_flat = prompt_mask.unsqueeze(1).expand(-1, M, -1).reshape(B * M, K)
        elif K > 0:
        ...
        if not inference:
            ...
            cot_input_embeds = token_embeds + chain_memory  # [B*M, L, d_model]
            ...
            if use_caching and K > 0:
                ...
                all_logits = decoder_outputs.logits  # [B*M, K+L, vocab_size] or [B*M, L, vocab_size]
                if prompt_embeds is not None:
    ...
    def _decode_simple_decoder(self, chain_memory, prompt_sequences, cot_sequences, prompt_mask, cot_mask, 
        ...
        if not inference:
            ...
            # TODO: IMPLEMENT EFFICIENT AUTO-REGRESSION FOR SIMPLE_DECODER
            # High-level proposition for efficient auto-regression in simple_decoder:
            ...
            for t in range(L):
                # Use generated tokens up to position t-1
                if t == 0:
                    # First position: use only prompt + latent
                    if K > 0:
                        prompt_embeds = self.decoder.transformer.wte(prompt_sequences)  # [B, K, d_model]
                        prompt_embeds = prompt_embeds.unsqueeze(1).expand(-1, M, -1, -1).reshape(B * M, K, d_model)
                        input_embeds = torch.cat([prompt_embeds, chain_memory[:, t:t+1, :]], dim=1)  # [B*M, K+1, d_model]
                        attn_mask = torch.ones(B * M, K+1, device=chain_memory.device)
                    else:
                        input_embeds = chain_memory[:, t:t+1, :]  # [B*M, 1, d_model]
                        attn_mask = torch.ones(B * M, 1, device=chain_memory.device)
                else:
                    # Subsequent positions: use prompt + generated tokens + current latent
                    if K > 0:
                        prompt_embeds = self.decoder.transformer.wte(prompt_sequences)  # [B, K, d_model]
                        prompt_embeds = prompt_embeds.unsqueeze(1).expand(-1, M, -1, -1).reshape(B * M, K, d_model)
                        generated_embeds = self.decoder.transformer.wte(output_sequences[:, :, :t].reshape(B * M, t))  # [B*M, t, d_model]
                        current_latent = chain_memory[:, t:t+1, :]  # [B*M, 1, d_model]
                        input_embeds = torch.cat([prompt_embeds, generated_embeds, current_latent], dim=1)  # [B*M, K+t+1, d_model]
                        attn_mask = torch.ones(B * M, K+t+1, device=chain_memory.device)
                    else:
                        generated_embeds = self.decoder.transformer.wte(output_sequences[:, :, :t].reshape(B * M, t))  # [B*M, t, d_model]
                        current_latent = chain_memory[:, t:t+1, :]  # [B*M, 1, d_model]
                        input_embeds = torch.cat([generated_embeds, current_latent], dim=1)  # [B*M, t+1, d_model]
                        attn_mask = torch.ones(B * M, t+1, device=chain_memory.device)
    ...
    def _decode_only_latent(self, chain_memory, prompt_sequences, cot_sequences, prompt_mask, cot_mask, 
        ...
        if not inference:
            ...
            # AUTO-REGRESSION MODE (INEFFICIENT IMPLEMENTATION)
            # TODO: IMPLEMENT EFFICIENT AUTO-REGRESSION FOR ONLY_LATENT_DECODE
            # High-level proposition for efficient auto-regression in only_latent_decode:
            # 1. Pre-compute prompt cache once
            # 2. For each position t, use the cached prompt + generated tokens up to t-1
            # 3. Add the latent embedding for position t
            # 4. Generate one token at position t
    ...
    def _decode_shared_helper(self, memory, prompt_sequences, cot_sequences, prompt_mask, cot_mask,
        ...
        """
        Shared helper function for decoding in both beam_search and interchain modes.
        ...
        Args:
            ...
            inference (bool): If True, performs auto-regressive generation
            pad_token_id (int): Token ID to use for padding
            use_caching (bool): Whether to use caching for efficiency
            mode (str): Either "beam_search" or "interchain"
            backpointers (torch.Tensor, optional): Backpointer indices for beam_search mode
            
        Returns:
            tuple or torch.Tensor: For beam_search mode: (output_logits, backpointer_logits)
                                  For interchain mode: output_logits
        """
        B, K = prompt_sequences.shape
        _, M, L = cot_sequences.shape
        ...
        if not inference:
            ...
            index_filler = torch.zeros((B, 1), dtype=beam_sequence.dtype, device=beam_sequence.device)
            beam_with_filler = torch.cat([filler, beam_sequence[:, :-1]], dim=1)  # [B, M*L]
            
            # Handle mode-specific indices and embeddings
            if mode == "beam_search":
                if backpointers is None:
                    raise ValueError("backpointers must be provided for beam_search mode")
                beam_indices = positionwise_flatten(backpointers)
                ...
                beam_indices = positionwise_flatten(chain_indices)  # [batch_size, M*L] each
                index_with_filler = torch.cat([index_filler, beam_indices[:, :-1]], dim=1)  # [B, M*L]
                # Create backpointer embeddings to add
                mode_embeds = self.chain_embeddings(index_with_filler)  # [batch_size, M*L, d_model]
            else:
                raise ValueError(f"Unsupported mode: {mode}")
            ...
            # Add latent embeddings
            cot_input_embeds = token_embeds + mode_embeds + memory  # [B, M*L, d_model]
            ...
            if mode == "beam_search":
                # For beam_search mode, also return backpointer logits
                ...
                # For interchain mode, only return token logits
                return cot_logits
            ...
            # First position: concatenate prompt embeddings with (filler embed + mode-specific embed + latent embed)
            filler = torch.full((B, 1), pad_token_id, dtype=torch.long, device=memory.device)  # [batch_size, 1]
            index_filler = torch.zeros((B, 1), dtype=torch.long, device=memory.device)  # [batch_size, 1]
            ...
            first_token_embeds = token_embeds + mode_embeds + latent_embeds  # [B, 1, d_model]
            ...
            if use_caching:
                ...
                for t in range(1, M*L):
                    ...
                    if mode == "beam_search":
                        generated_indices = torch.argmax(decoder_outputs.backpointer_logits, dim=-1)  # [B, 1]
    ...
    def _decode_normal(self, chain_memory, prompt_sequences, cot_sequences, prompt_mask, cot_mask, 
        ...
        if not inference:
            ...
            if use_caching:
                ...
                full_combined_mask = self._create_combined_mask(prompt_mask, cot_mask_flat, B, M, K, L, cot_sequences.device)
                ...
                cross_attention_mask = cross_attention_mask.unsqueeze(0).unsqueeze(0).expand(
                    B * M, -1, -1, -1) # [batch_size*M, 1, L+1, L]
            ...
            # AUTO-REGRESSION MODE (INEFFICIENT IMPLEMENTATION)
            # TODO: IMPLEMENT EFFICIENT AUTO-REGRESSION FOR NORMAL_DECODE
            # High-level proposition for efficient auto-regression in normal_decode:
            # 1. Pre-compute prompt cache once
            # 2. For each position t, use the cached prompt + generated tokens up to t-1
            # 3. Add the latent embedding for position t
            # 4. Generate one token at position t
    ...
    def from_checkpoint(cls, checkpoint_path: str, device: Optional[str] = None, **kwargs):
        """
        Initialize and load a GPT2VQVAE model from a checkpoint file.
        ...
        Returns:
            GPT2VQVAE: Fully initialized and loaded model
        ...
        Example:
            >>> # Load model with checkpoint configuration
            >>> model = GPT2VQVAE.from_checkpoint('checkpoints/model_epoch_10.pt')
            
            >>> # Load model with overridden parameters
            >>> model = GPT2VQVAE.from_checkpoint(
            ...     'checkpoints/model_epoch_10.pt',
            ...     device='cpu',
            ...     use_pretrained_encoder=False
            ... )
        """
        return cls._from_checkpoint_impl(checkpoint_path, device, **kwargs)
    
    @classmethod
    def _from_checkpoint_impl(cls, checkpoint_path: str, device: Optional[str] = None, 
                             model_config: Optional[dict] = None, **kwargs):
        """
        Internal implementation of from_checkpoint that can be overridden by subclasses.
        
        Args:
            checkpoint_path (str): Path to the checkpoint file
            device (str, optional): Device to load the model on
            model_config (dict, optional): Pre-processed model configuration
            **kwargs: Additional arguments to override the checkpoint configuration
            
        Returns:
            Model instance: Fully initialized and loaded model
        """
        if device is None:
        ...
        required_fields = ['vocab_size', 'd_model', 'num_embeddings', 'commitment_cost', 
                           'aggregation_hidden_dim', 'num_thoughts', 'n_positions', 
        ...
        print(f"Initializing {cls.__name__} model from checkpoint configuration...")
        ...
        print(f"Successfully loaded {cls.__name__} model from checkpoint: {checkpoint_path}")
    ...
    def _get_prompt_cache(self, decoder, prompt_sequences, prompt_mask, exclude_last_token=False, pad_token_id=0):
        ...
        Returns the prompt cache (and prompt activations if needed).
        """
        if exclude_last_token and prompt_sequences.shape[1] > 1:
            prompt_input = prompt_sequences[:, :-1]
            prompt_mask_input = prompt_mask[:, :-1] if prompt_mask is not None else None
        elif exclude_last_token and prompt_sequences.shape[1] == 1:
            # If K=1, start with empty cache
            return EncoderDecoderCache(DynamicCache(), DynamicCache())
        else:
            prompt_input = prompt_sequences
